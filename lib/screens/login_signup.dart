...
void initState() {
  ...
  void authenticateFirebase(BuildContext context) async {
     setState(() {
      _isLoading=true;
    });
       
    if(!_isLogin&& _enteredUserName.trim().isNotEmpty){
        final _authResponse=await _firebaseAuth.createUserWithEmailAndPassword(
         email: _enteredEmail,
         password: _enteredPassword);

         final response=await FirebaseFirestore.instance.collection('users').doc(_authResponse.user!.uid).set({
         ...
         .then((_) {
         validateUserRoleAndNavigate(context, widget.role); // Pass the expected role to compare
      });

       }
  }

  Future<void> signInWithGoogleAndSaveToFirestore(BuildContext context, String expectedRole) async {
  try {
    setState(() {
      _isLoading = true;
    });
    ...
    if (user != null) {
      ...
      if (!snapshot.exists) {
        // New user - save data with the selected role
        await userDoc.set({
          'uid': uid,
          'name': name,
          'email': email,
      ...
      if (data != null && data['role'] != expectedRole) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('You are trying to log in through the wrong portal. Please use the correct role login.'),
...
String getLoadingMessage(String role) {
  ...
  // );
      ...
      _isLoading=false;
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(error.toString() ?? 'Invalid Authentication'),),
 ...
 Future<void> validateUserRoleAndNavigate(BuildContext context, String expectedRole) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        backgroundColor: Colors.redAccent,
         ...
         Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => AuthChecker(),));
               ...
               Form(
                 ...
                 // ),
                     ...
                     if (!_isLogin)
                       TextFormField(
                         decoration: InputDecoration(
                           ...
                           color: primaryColor.withAlpha(180), // your desired focus color
                           width: 2.0,
                            ),
                     ...
                     TextFormField(
                       ...
                       decoration: InputDecoration(
                         ...
                         suffixIcon: IconButton(
                           ...
                           icon: _togglePassword?Icon(Icons.visibility_off):Icon(Icons.visibility)),
                         border: OutlineInputBorder(
                           borderRadius: BorderRadius.all(Radius.circular(10)),
                         ),
                       ),
                     ...
                     children: [
                       Expanded(
                         child: ElevatedButton(
                           ...
                           child: Text(_isLogin ? 'Log In' : 'Sign Up',style: TextStyle(color:Colors.white,fontSize: 16),),
                   ...
                   Row(
                    ...
                    child: Text("Or Log in with",style:TextStyle(fontSize: 16)),
                    ),
                    Expanded(
                    child: Divider(
                    thickness: 1,
                   ...
                   Row(
                     ...
                     children: [
                       IconButton.filled(
                         onPressed:(){
                         signInWithGoogleAndSaveToFirestore(context,title);
                         },
                   ...
                   Row(
                     ...
                     children: [
                       ...
                       TextButton(
                         ...
                         child: Text(_isLogin ? 'Sign up' : 'Login',style: Theme.of(context)
                         .textTheme
                         .bodySmall!
                         .copyWith(color: primaryColor, fontWeight: FontWeight.bold),),
                       ),
         ...
         if (_isLoading)
            Positioned.fill(
              child: Container(
                ...
                child: Center(
                  child:ModernLoadingScreen(message:getLoadingMessage(widget.role)),
